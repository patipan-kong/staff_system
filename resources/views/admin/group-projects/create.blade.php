...
<div class="row">
    <div class="col-12 d-flex justify-content-between align-items-center mb-4">
        <h1><i class="fas fa-plus"></i> Create Group Project</h1>
        <a href="{{ route('admin.group-projects.index') }}" class="btn btn-outline-secondary">
            <i class="fas fa-arrow-left"></i> Back to Group Projects
        </a>
    </div>
</div>
...
<div class="row justify-content-center">
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.group-projects.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  ...
                                  placeholder="Brief description of the group project and its objectives...">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('icon') is-invalid @enderror" id="icon" name="icon">
                                ...
                                <option value="fas fa-project-diagram" {{ old('icon') == 'fas fa-project-diagram' ? 'selected' : '' }}>📊 Project Diagram</option>
                                ...
                                <option value="fas fa-globe" {{ old('icon') == 'fas fa-globe' ? 'selected' : '' }}>🌐 Web</option>
                                ...
                                <option value="fas fa-cog" {{ old('icon') == 'fas fa-cog' ? 'selected' : '' }}>⚙️ System</option>
                                <option value="fas fa-rocket" {{ old('icon') == 'fas fa-rocket' ? 'selected' : '' }}>🚀 Launch</option>
                                <option value="fas fa-lightbulb" {{ old('icon') == 'fas fa-lightbulb' ? 'selected' : '' }}>💡 Innovation</option>
                            </select>
                            @error('icon')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-3">
                        <div class="card bg-light">
                            <div class="card-body text-center">
                                <h6>Preview</h6>
                                <div class="d-inline-block p-3 rounded" style="background-color: #007bff20; border: 2px solid #007bff" id="preview-card">
                                    <i class="fas fa-folder fa-2x mb-2" style="color: #007bff" id="preview-icon"></i>
                                    <h6 id="preview-name">Group Project Name</h6>
                                    <span class="badge bg-info" id="preview-status">Status</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('admin.group-projects.index') }}" class="btn btn-secondary">
                            <i class="fas fa-times"></i> Cancel
                        </a>
                        <button type="submit" class="btn btn-success">
                            <i class="fas fa-save"></i> Create Group Project
                        </button>
                    </div>
                </form>
            </div>
